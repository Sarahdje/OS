...
bootloader:
	...
	ld -T "${bootloader}"/linker.ld "${bootloader}"/early_boot.o "${bootloader}"/main.o --oformat binary -o "${bootloader}"/early_boot

bootdisk: bootloader
	dd if=/dev/zero of=disk.img bs=512 count=2880
	dd conv=notrunc if="${bootloader}"/bootloader of=disk.img bs=512 count=1 seek=0
	dd conv=notrunc if="${bootloader}"/early_boot of=disk.img bs=512 count=8 seek=1
